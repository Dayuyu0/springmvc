...
public class ResumeDaoTest {
    ...
    public void testFindOne(){
        ...
        resume.setId(1l);
        //Example.of方法将resume转换成条件对象，并传入
    ...
    public void testSort(){
        //实例化sort，第一个参数排序规则，第二个参数按照哪个属性进行排序
    ...
    public void testPage(){
        //实例化pageable，第一个参数当前页，从0开始
        // 第二个参数查询数量
        Pageable pageable = PageRequest.of(0, 2);
        //指定规则排序，传入pageable
        Page<Resume> all = resumeDao.findAll(pageable);
        System.out.println("当前共"+all.getTotalPages()+"页");
        System.out.println("当前共"+all.getTotalElements()+"条数据");
    ...
    @Test
    public void testsql(){
        List<Resume> bySql = resumeDao.findBySql("张%", "北%");
        for (int i = 0; i < bySql.size(); i++) {
            Resume resume =  bySql.get(i);
            System.out.println(resume);
        }
    }
    ...
    public void testMethodName(){
        List<Resume> bySql = resumeDao.findByNameLikeAndAddress("张%", "北京");
        for (int i = 0; i < bySql.size(); i++) {
            Resume resume =  bySql.get(i);
            System.out.println(resume);
        }
    ...
    public void testSpecificationMultiCon(){
        ...
        /**
         * 封装动态条件
         *      需求：根据name（指定为"张三"）,address以 北 开头查询简历
         */
        //
        Specification<Resume> specification = new Specification<Resume>() {
            @Override
